-- Fix existing policies and create missing triggers for user_summary

-- Drop existing policies that might be blocking changes
...
-- Create policy to allow triggers to modify user_summary (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        ...
        AND policyname = 'Only triggers can modify user summary'
    ) THEN
